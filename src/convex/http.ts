...
const asErrorMessage = (error: unknown) => (error instanceof Error ? error.message : "Unknown error");
...
function parseEventPayload(input: unknown, path = "event"): EventPayload {
  ...
  return {
    ...
    gender_distribution: parseGenderDistribution(record.gender_distribution, `${path}.gender_distribution`),
...
function parseUserPayload(input: unknown): UserPayload {
  ...
  return {
    ...
    total_age_distribution: parseAgeDistribution(record.total_age_distribution, "user.total_age_distribution"),
    ...
    try {
      ...
      for (const entry of payload.events) {
        if (typeof entry === "string") {
          ...
          if (!existing) {
            throw new Error(`Unknown event "${entry}". Include the full event object to create it.`);
          }
